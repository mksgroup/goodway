...
import mksgroup.goodway.user.domain.repository.UserRepository;
...
public class UserServiceImpl extends BaseService<User, String>
    ...
    public void add(User user) throws Exception {
        if (userRepository.containsName(user.getName())) {
            throw new Exception(String.format("There is already a rquest with the name - %s", user.getName()));
        }

        if (user.getName() == null || "".equals(user.getName())) {
            throw new Exception("Requesting name cannot be null or empty string.");
        }
        super.add(user);
    }
